...
import { NAVIGATION_CONFIG, WIKI_NAVIGATION } from "@/config/navigation";
...
export async function WikiSidebar({ locale }: { locale: string }) {
  ...
  return <aside className="space-y-6 lg:sticky lg:top-24"><section className="rounded-2xl border border-border bg-card/60 p-5 shadow-sm"><h3 className="mb-4 text-xs font-bold uppercase tracking-[0.22em] text-muted-foreground">{t("wikiNavigation")}</h3><div className="space-y-4">{WIKI_NAVIGATION.map((section) => <div key={section.title}><div className="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground"><section.icon className="h-4 w-4 text-[hsl(var(--nav-theme))]" /><span>{section.title}</span><span className="ml-auto text-xs text-muted-foreground">{section.count}</span></div><ul className="space-y-1">{section.links.slice(0, section.title === "Bosses" ? 29 : 8).map(([label, href, badge]) => <li key={href}><Link href={localizeHref(href, locale)} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"><span className="truncate">{label}</span>{badge && <Badge variant="secondary" className="ml-auto h-5 border-border px-1.5 text-[10px]">{badge}</Badge>}</Link></li>)}</ul></div>)}</div></section><section className="rounded-2xl border border-border bg-card/60 p-5"><h3 className="mb-3 text-sm font-bold text-foreground">{t("activeCodes")}</h3><div className="space-y-3 text-sm"><div className="rounded-xl bg-muted p-3"><code className="font-bold text-foreground">FULLRELEASE</code><p className="mt-1 text-muted-foreground">1x Manipulator's Eyepatch (Limited) + 10x Clan Reroll</p></div><div className="rounded-xl bg-muted p-3"><code className="font-bold text-foreground">75KLIKES</code><p className="mt-1 text-muted-foreground">3x Ability Reroll</p></div><Link href={localizeHref("/codes", locale)} className="inline-flex items-center gap-1 text-sm font-semibold text-[hsl(var(--nav-theme))]">{t("viewAllCodes")} <ChevronRight className="h-4 w-4" /></Link></div></section></aside>;
}
